...
class MockZoneInfo: Codable {
    let zoneId: String
    let color: UIColor?
    let pirceColor: UIColor?
    let bounds: CGRect
    init(zoneId: String, bounds: CGRect, color: UIColor? = nil, pirceColor: UIColor? = nil) {
        self.zoneId = zoneId
        self.bounds = bounds
        self.color = color
    ...
    required init(from decoder: Decoder) throws {
        ...
        pirceColor = try container.decodeIfPresent(String.self, forKey: .pirceColor).flatMap { UIColor.color(from: $0) }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        if let pirceColor {
            try container.encode(pirceColor.rgbaHex, forKey: .pirceColor)
        }
