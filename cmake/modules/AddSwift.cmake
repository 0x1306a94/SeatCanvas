...
# Generate the bridging header from Swift to C++ with specified source files
#
# target: the name of the target to generate headers for.
# header: the name of the header file to generate.
# SWIFT_SOURCES: explicitly specified list of Swift source files.
#
# Usage:
#   _swift_generate_cxx_header_from_sources(MyTarget "MyHeader.h" 
#     SWIFT_SOURCES source1.swift source2.swift
#     MODULE_NAME MyModule
#     SEARCH_PATHS /path/to/include)
function(_swift_generate_cxx_header_from_sources target header)
  ...
  cmake_parse_arguments(ARG "" "" "SEARCH_PATHS;MODULE_NAME;SWIFT_SOURCES" ${ARGN})

  if(NOT ARG_SWIFT_SOURCES)
    message(FATAL_ERROR "SWIFT_SOURCES must be specified for _swift_generate_cxx_header_from_sources")
  endif()
  ...
  if(APPLE AND CMAKE_OSX_SYSROOT)
    set(SDK_FLAGS "-target" "${APPLE_TARGET_TRIPLE}" "-sdk" "${CMAKE_OSX_SYSROOT}")
  elseif(WIN32)
  ...
  set(_SwiftSources "")
  foreach(swift_source ${ARG_SWIFT_SOURCES})
    cmake_path(IS_ABSOLUTE swift_source is_absolute)
    if(NOT is_absolute)
      cmake_path(APPEND CMAKE_CURRENT_SOURCE_DIR ${swift_source} OUTPUT_VARIABLE absolute_source)
      list(APPEND _SwiftSources ${absolute_source})
    else()
      list(APPEND _SwiftSources ${swift_source})
    endif()
  endforeach()
  ...
  add_custom_command(OUTPUT ${header_path}
    ...
    COMMENT
      "Generating '${header_path}' from specified Swift sources"
