...
class BaseMapMeshBuilder {
  public:
    ...
    std::shared_ptr<ZoneMeshInfo> findZoneById(const std::string &zoneId) const;

    /// 查找与指定矩形相交的所有区域
    /// @param rect 查询矩形
    /// @param visibleIndices 索引
    /// @return 相交的区域网格信息列表
    std::vector<std::shared_ptr<ZoneMeshInfo>> findZoneIntersectingRect(const tgfx::Rect &rect, std::vector<size_t> *visibleIndices = nullptr) const;

    /// 查找包含指定点的区域
    /// @param point 查询点（SVG 坐标）
    /// @return 包含该点的区域网格信息，如果不存在则返回 nullptr（返回最上层的区域）
    std::shared_ptr<ZoneMeshInfo> findZoneContainingPoint(const tgfx::Point &point) const;
