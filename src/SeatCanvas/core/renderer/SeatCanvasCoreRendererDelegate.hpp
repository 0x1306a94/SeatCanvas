...
class SeatCanvasCoreRendererDelegate {
  public:
    ...
    virtual bool shouldSelectSeat(uint32_t coreID, const std::string &seatId) = 0;
    virtual void didSelectSeat(uint32_t coreID, const std::string &seatId) = 0;
    virtual void didDeselectSeat(uint32_t coreID, const std::string &seatId) = 0;
};
};  // namespace kk::renderer
